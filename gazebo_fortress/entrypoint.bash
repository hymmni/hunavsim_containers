...
while true; do
    ...
    if [[ $opt =~ ^[0-9]+$ ]]; then
        ...
        if [ $idx -ge 0 ] && [ $idx -lt ${#yaml_files[@]} ]; then
            ...
            echo ""
            echo "Launching..."
            echo -e "\e[33mros2 launch hunav_gazebo_fortress_wrapper simulation_fortress.launch.py environment_name:=$map_name configuration_file:=$yaml_name\e[0m"
            echo ""
            ros2 launch hunav_gazebo_fortress_wrapper simulation_fortress.launch.py environment_name:=$map_name configuration_file:=$yaml_name
            ...
            echo -e "\e[3m\e[32mdocker exec -it hunavsim_gz_fortress bash\e[0m"
        elif [ "$opt" -eq "$rviz_option" ]; then
            MAPS_DIR="/home/hunav_gz_fortress_ws/src/hunav_gazebo_fortress_wrapper/maps"
            ...
            if [[ $map_opt =~ ^[0-9]+$ ]]; then
                ...
                if [ $id -ge 0 ] && [ $id -lt ${#map_files[@]} ]; then
                    ...
                    echo ""
                    ros2 launch hunav_rviz2_panel hunav_rviz2_launch.py wrapper_pkg:=hunav_gazebo_fortress_wrapper map:=$map_name
                    # Compile the workspace in case a new scenario was created
                    echo "Compiling the workspace..."
                    cd /home/hunav_gz_fortress_ws
                    if ! colcon build --symlink-install; then
                        echo "Error: the workspace compilation failed! Stopping the script and opening a bash shell."
